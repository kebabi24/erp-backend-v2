...
const create = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const devise = await deviseServiceInstance.create({...req.body, cu_domain:user_domain,created_by:user_code,created_ip_adr: req.headers.origin, last_modified_by:user_code,last_modified_ip_adr: req.headers.origin})
        return res
            .status(201)
...
const findAll = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const deviseServiceInstance = Container.get(DeviseService)
        const devise = await deviseServiceInstance.find({cu_domain:user_domain})
        return res
            .status(200)
            .json({ message: "fetched succesfully", data: devise })
    } catch (e) {
...
const findBy = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        const deviseServiceInstance = Container.get(DeviseService)
        const devise = await deviseServiceInstance.findOne({...req.body,cu_domain:user_domain})
        return res
            .status(200)
            .json({ message: "fetched succesfully", data: devise })
    } catch (e) {
