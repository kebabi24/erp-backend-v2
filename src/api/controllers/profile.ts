...
const create = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    const profile = await profileServiceInstance.create({
      ...req.body,
      usrg_domain: user_domain,
      created_by: user_code,
      created_ip_adr: req.headers.origin,
      last_modified_by: user_code,
      last_modified_ip_adr: req.headers.origin,
    });
    return res.status(201).json({ message: 'created succesfully', data: profile });
  } catch (e) {
...
const findOne = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    console.log(profile);
    return res.status(200).json({ message: 'fetched succesfully', data: profile });
  } catch (e) {
    logger.error('🔥 error: %o', e);
    return next(e);
  }
